...
<body>
    ...
    <table>
        ...
        @foreach($invoice->items as $it)
            <tr>
                ...
                <td class="right">{{ number_format((float)$it->amount, 2, ',', ' ') }} FCFA</td>
            </tr>
        @endforeach
        </tbody>
    </table>
    ...
    <div style="margin-top: 16px;">
        <div><span class="muted">Sous-total</span> {{ number_format((float)$invoice->subtotal, 2, ',', ' ') }} FCFA</div>
        <div><span class="muted">Remise</span> {{ number_format((float)$invoice->discount_amount, 2, ',', ' ') }} FCFA</div>
        <div><span class="muted">Taxes</span> {{ number_format((float)$invoice->tax_amount, 2, ',', ' ') }} FCFA</div>
        <div><b>Total</b> {{ number_format((float)$invoice->total_amount, 2, ',', ' ') }} FCFA</div>
        <div><span class="muted">Payé</span> {{ number_format((float)$invoice->amount_paid, 2, ',', ' ') }} FCFA</div>
        <div><b>Reste dû</b> {{ number_format((float)$invoice->amount_due, 2, ',', ' ') }} FCFA</div>
    </div>
    </div>
</body>
</html>
